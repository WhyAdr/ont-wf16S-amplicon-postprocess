#!/usr/bin/env Rscript
# =============================================================================
# wf-16s amplicon results - downstream R analysis
...
# =============================================================================
ab_raw <- read_tsv("data/abundance_table_species.tsv", show_col_types = FALSE)
...
n_unclassified <- ab$count[ab$is_unclassified]
...
# =============================================================================
reads <- read_tsv(
  "data/AmbarAyunda_minimap2_16S_lineages_minimap2_assignments.tsv",
  col_names = c("status","read_id","taxid","len_field","lineage"),
  show_col_types = FALSE
) %>%
  mutate(
    read_length = as.integer(str_extract(len_field, "(?<=\\|)\\d+")),
    # IMPORTANT: raw `status` (col 1) reflects whether the read aligned at all.
    # But wf-16s's post-alignment QC filter (min_ref_coverage / min_percent_identity)
    # zeroes out `taxid` (col 3) for reads that aligned but failed coverage/identity
    # thresholds, WITHOUT updating the status letter itself (csvtk replace only
    # touches field 3). So `status == "C"` alone overcounts true classification.
    # The taxonomically correct label requires taxid != 0 as well.
...
donut_df <- reads %>%
  ...
  mutate(
    ...
    label = sprintf("%s\n%s reads (%.1f%%)", status_label_corrected, comma(n), 100*frac)
  )
...
p1a <- ggplot(donut_df, aes(ymin = ymin, ymax = ymax, xmin = 3, xmax = 4, fill = status_label_corrected)) +
  ...
  geom_text(data = donut_df, aes(x = 1, y = 0, label = sprintf("%s\nreads", comma(sum(n)))),
  ...
  geom_label(aes(x = 3.5, y = (ymin+ymax)/2, label = label), inherit.aes = FALSE,
             data = donut_df, fill = "white", color = "black", size = 3.3,
             label.size = 0)
...
# =============================================================================
# FIGURE 2 — Phylum-level composition (donut/bar)
# =============================================================================
phylum_comp_full <- ab %>%
...
write_csv(phylum_comp_full, "tables/phylum_composition.csv")

# Collapse anything below 0.4% into "Other phyla" for a readable plot (full table still saved above)
N_KEEP <- 7
phylum_comp <- phylum_comp_full %>%
  ...
  arrange(desc(count)) %>%
  mutate(phylum_clean = fct_reorder(phylum_clean, count))
...
top_species <- ab %>%
  ...
  arrange(desc(count)) %>%
  slice_head(n = 15) %>%
  mutate(species_lab = fct_reorder(species, count))
...
p4 <- ggplot(top_species, aes(x = species_lab, y = rel_abund, fill = phylum)) +
  ...
  labs(title = "Top 15 species by relative abundance",
       subtitle = sprintf("Out of %d species-level taxa detected", sum(!ab$is_unclassified)),
       x = NULL, y = "Relative abundance (of total reads)") +
  theme_amplicon +
...
S <- length(classified_counts)
shannon <- diversity(classified_counts, index = "shannon")
simpson <- diversity(classified_counts, index = "simpson")
invsimpson <- diversity(classified_counts, index = "invsimpson")
pielou <- shannon / log(S)
ens <- exp(shannon)
bp_dominance <- max(classified_counts) / sum(classified_counts)
chao1 <- tryCatch(estimateR(classified_counts)["S.chao1"], error = function(e) NA)
fisher_alpha <- tryCatch(fisher.alpha(classified_counts), error = function(e) NA)
...
write_csv(alpha_tbl, "tables/alpha_diversity.csv")
...
# =============================================================================
set.seed(4)
depth_points <- unique(round(seq(100, sum(classified_counts), length.out = 25)))
n_boot <- 15

rare_results <- lapply(depth_points, function(d) {
  reps <- sapply(seq_len(n_boot), function(i) {
    length(rrarefy(matrix(classified_counts, nrow=1), d)[1, ] %>% .[. > 0])
  })
  tibble(depth = d, mean_richness = mean(reps), sd_richness = sd(reps))
}) %>% bind_rows()
...
p5 <- ggplot(rare_results, aes(x = depth, y = mean_richness)) +
  ...
  geom_vline(xintercept = sum(classified_counts), linetype = "dashed", color = "grey50") +
  annotate("text", x = sum(classified_counts), y = 50,
  ...
  labs(title = "Within-sample rarefaction curve",
       subtitle = "Species richness vs. sequencing depth (15 resamples per depth, mean \u00b1 SD)",
...
# Quick summary stat for that comparison, printed to console
len_summary <- reads %>%
...
write_csv(len_summary, "tables/read_length_by_status.csv")
...
write_csv(richness_per_rank, "tables/richness_per_rank.csv")
