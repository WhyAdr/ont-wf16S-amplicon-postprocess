...
Version 0.3.0 is validated against the tracked `wf-16s` minimap2 abundance,
...
- `input.params_json` is required. Version 0.3.0 accepts minimap2, species-rank
...
session information. These capture provenance but do not by themselves recreate
an environment. Version 0.3.0 is therefore provenance-captured, not bitwise
environment-locked. A release lock will be generated from the final green Ubuntu
reference environment; publication analyses should create and archive their own
project-specific lock until then.
...
Version 0.3.0 covers validated NCBI/minimap2/species parsing, exact read accounting, single-sample summaries, synthetically
