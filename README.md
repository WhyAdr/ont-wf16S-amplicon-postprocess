...
Current pipeline version: **0.4.0**.
...
Version 0.4.0 is validated against the tracked `wf-16s` minimap2 abundance,
...
- `input.params_json` is required. Version 0.4.0 accepts minimap2, species-rank
...
Use a new output root for publication runs. `--overwrite` permits current files
to replace same-named outputs, but it does not remove stale artifacts from an
earlier run.
...
and `microeco`; use `renv::restore(prompt = FALSE)` before release or publication
runs. `analysis/install_packages.R --restore` provides the same explicit locked
setup path. The lock covers this post-processing environment, not the upstream
`wf-16s` workflow, ONT basecaller, reference database archive, or optional
KronaTools executable.

### Run-manifest schema v2

Version 0.4.0 introduces manifest schema v2; this is a machine-readable shape
...
Version 0.4.0 covers validated NCBI/minimap2/species parsing, exact read accounting, single-sample summaries, synthetically
