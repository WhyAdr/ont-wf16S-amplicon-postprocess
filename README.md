...
Current pipeline version: **0.4.2**.
...
Version 0.4.2 is validated against the tracked `wf-16s` minimap2 abundance,
...
- `input.params_json` is required. Version 0.4.2 accepts minimap2, species-rank
...
Version 0.4.2 maintains manifest schema v2 (revision 1); this is a machine-readable shape
change, not an analytical-denominator or output-semantics change. Cardinality-
dependent fields such as samples, requested modules, package versions, input
collections, module outputs/warnings, and Krona provenance samples are always
...
Version 0.4.2 covers validated NCBI/minimap2/species parsing, exact read accounting, single-sample summaries, synthetically
...
(via locked `microeco 2.3.0`). It also includes manifest schema v2 and bounded
assignment parsing. The project does
