...
Version 0.4.3 is validated against the tracked `wf-16s` minimap2 abundance,
...
- `input.params_json` is required. Version 0.4.3 accepts minimap2, species-rank
...
accepted by KronaTools `ktImportText`. When `render_html: true` and
KronaTools is available, the same module also writes
`<sample>.krona.html`; without the external renderer, the TSV is retained,
the run records a renderer-missing warning, and no HTML artifact is claimed.
The module writes `07_Kreport/krona/krona_provenance.json` with the exact
read-accounting reconciliation and renderer status.
...
Version 0.4.3 advances to manifest schema v2 (revision 2); this maintains the machine-readable shape
...
| `--krona` | Enable Krona TSV export and optional KronaTools HTML rendering (requires `kreport`) |
...
Version 0.4.3 covers validated NCBI/minimap2/species parsing, exact read accounting, single-sample summaries, synthetically
tested cohort modules, offline-by-default taxonomy resolution, Pavian-ready
Kraken reports, opt-in Krona-compatible TSV export with optional KronaTools
HTML rendering, and an opt-in FAPROTAX 1.2.12 functional inference module
(via locked `microeco 2.3.0`). It also includes manifest schema v2 revision 2, bounded
assignment parsing, physical output census verification, transaction journals, and multi-process
cache locking. The project does
...
statistics or species-level calls. Krona HTML remains unavailable unless
KronaTools `ktImportText` is installed and rendering is enabled.
